...
For JavaScript we use ES6 modules. Each component is defined as a class with a mandatory init method.
...
To execute ``MyComponent`` you need to define a ``data-module`` attribute inside HTL template file.
...
By default ``data-module`` will load a separate chunk for your component at runtime. Yep! We don't need to write script tags for each bundle we create. This is good for scenarios where chunks are lazy loaded based on conditions.<br><br>
