...
public class ErrorModelTest {
    ...
    public void testGetStartedMessage() {
        ...
        assertNotNull("not null", heading);
        Assert.assertEquals("Oops... Page Not found", "Oops... Page Not found", errorModel.getErrorTitle());
        Assert.assertEquals("404", "404", errorModel.getErrorCode());
        String imagePath = errorModel.getImagePath();
        assertNotNull("not null", imagePath);
        Assert.assertEquals("Error Cuhu", "Error Cuhu", errorModel.getImageAltText());
        assertNotNull("not null", errorModel.getErrorDescription());
    }
