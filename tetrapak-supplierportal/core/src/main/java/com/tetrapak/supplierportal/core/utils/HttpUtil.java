...
public final class HttpUtil {
	...
	public static JsonObject sendAPIGeePostWithFiles(String url, String token, File file) {
		...
		HttpEntity requestEntity = MultipartEntityBuilder.create().addPart(FILE_PARAM_NAME, uploadFilePart).build();
	...
	private static JsonObject executeRequest(HttpPost apiRequest) {
		...
		try {
			...
			jsonResponse = HttpUtil.setJsonResponse(jsonResponse, "Unsupported Encoding",
					HttpStatus.SC_INTERNAL_SERVER_ERROR);
			LOGGER.error("Unsupported Encoding while sending post request" + e);
		} catch (ClientProtocolException e) {
			jsonResponse = HttpUtil.setJsonResponse(jsonResponse, "ClientProtocolException Error",
					HttpStatus.SC_INTERNAL_SERVER_ERROR);
	...
	/**
	 * 
	 * This method is a variation of getJsonObject provided in the same file. This
	 * method does not parse the API response into JSON Object; instead it returns
	 * the response AS IS in the form of a string.
	 * 
	...
	public static Map<String, String> executeHttp(String token, String url) {
		...
		try {
			...
			responseMap.put(SupplierPortalConstants.RESULT, responseString);
			responseMap.put(SupplierPortalConstants.STATUS,
