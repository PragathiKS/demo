		    ...
		    steps {
			   parallel (
			       "Author Deployment" : {
			          ...
			          sh "curl -u admin:admin -F name=${params.CHOICE}.complete -F file=@$workspace/${params.CHOICE}/complete/target/${params.CHOICE}.complete-1.0.0-SNAPSHOT-${BUILD_NUMBER}.zip -F force=true '${author_url}/crx/packmgr/service.jsp?cmd=upload' --verbose"
		            ...
		            "Publish_Deployment" : {
			            ...
			            echo "Uploading New Package"
			            sh "curl -u admin:admin -F name=${params.CHOICE}.complete -F file=@$workspace/${params.CHOICE}/complete/target/${params.CHOICE}.complete-1.0.0-SNAPSHOT-${BUILD_NUMBER}.zip -F force=true '${publish_url}/crx/packmgr/service.jsp?cmd=upload' --verbose"
			            echo "Installing New Package"
